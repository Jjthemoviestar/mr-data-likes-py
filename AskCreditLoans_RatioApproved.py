...
import numpy as np
import pandas as pd
import F1_import  #importing import code

#print (loan_data.dtypes)

loan_data['Loan_Status'].str.contains('Y') #which rows have a Y

loan_data['Loan_Status'] == 'Y' #which rows have a Y
loan_data = f1.loan_data.fillna('blank') #replaces missing data with blank string

#loan_data.head()  #useful for when looking at a new DF
 
ln_status_index = (loan_data['Loan_Status'] == 'Y') #creates subset of rows with Y

ln_status_Y = loan_data[ln_status_index] #creates subset of true values
...
ln_status_index['Credit_History'].value_counts().plot(kind='pie')
